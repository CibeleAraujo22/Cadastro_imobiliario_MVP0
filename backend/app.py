...
app = Flask(__name__)
#conexão com o banco do postgresql
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:postgres@localhost:5432/Imobiliaria_api'
CORS(app)
cross_origin(app = app, methods=['GET', 'POST', 'PUT', 'DELETE'])
appi = Api(app=app, version="1.0", title="Sistema de Imóveis Campinas Tech Talents--Enforce",
           description="Sistema MVP_0 de Compra e Venda de Imóveis")
db = SQLAlchemy(app)

#Criando os NAMESPACE & MODELS:
#clientes
...
model_get_cliente = appi.model(
    'Cliente Model', {'Id': fields.Integer(description="Id do cliente")})
